...
function DbNode({ id, data }) {
  ...
  const onNameChange = useCallback((evt) => {
    setNodes((nds) => nds.map((n) => n.id === id ? { ...n, data: { ...n.data, tableName: evt.target.value } } : n));
  }, [id, setNodes]);

  const toggleTimestamps = () => {
    data.takeSnapshot?.();
    setNodes((nds) => nds.map((n) => n.id === id ? { ...n, data: { ...n.data, timestamps: !data.timestamps } } : n));
  };
  ...
  const handleSaveField = (fieldData) => {
    ...
    setNodes((nds) => nds.map((n) => n.id === id ? { ...n, data: { ...n.data, fields: currentFields } } : n));
    setShowForm(false);
